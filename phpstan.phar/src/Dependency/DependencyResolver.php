...
class DependencyResolver
{
    ...
    public function resolveDependencies(Node $node, Scope $scope) : \PHPStan\Dependency\NodeDependencies
    {
        ...
        if ($node instanceof InClassNode || $node instanceof InTraitNode) {
            ...
            if ($docComment !== null) {
                $phpDoc = $this->fileTypeMapper->getResolvedPhpDoc($scope->getFile(), $scope->isInClass() ? $scope->getClassReflection()->getName() : null, $scope->isInTrait() ? $scope->getTraitReflection()->getName() : null, null, $docComment->getText());

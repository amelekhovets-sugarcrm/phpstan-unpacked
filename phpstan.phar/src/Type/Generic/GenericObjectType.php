...
class GenericObjectType extends ObjectType
{
    ...
    {
        return sprintf('%s<%s>', parent::describe($level), implode(', ', array_map(static function (Type $type, ?\PHPStan\Type\Generic\TemplateTypeVariance $variance = null) use($level) : string {
            return \PHPStan\Type\Generic\TypeProjectionHelper::describe($type, $variance, $level);
        }, $this->types, $this->variances)));
    }
    ...
    {
        return new self($properties['className'], $properties['types'], $properties['subtractedType'] ?? null, null, $properties['variances'] ?? []);
    }
}

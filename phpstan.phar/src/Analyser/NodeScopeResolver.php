...
class NodeScopeResolver
{
    ...
    public function processNodes(array $nodes, \PHPStan\Analyser\MutatingScope $scope, callable $nodeCallback) : void
    {
        foreach ($nodes as $i => $node) {
            ...
            $nextStmt = $this->getFirstUnreachableNode(array_slice($nodes, $i + 1), \true);
    ...
    {
        if (!$stmt instanceof Static_ && !$stmt instanceof Foreach_ && !$stmt instanceof Node\Stmt\Global_ && !$stmt instanceof Node\Stmt\Property && !$stmt instanceof Node\Stmt\PropertyProperty && !$stmt instanceof Node\Stmt\ClassConst && !$stmt instanceof Node\Stmt\Const_) {
        ...
        if ($stmt instanceof Node\Stmt\Declare_) {
            ...
            return new \PHPStan\Analyser\StatementResult($scope, $hasYield, $alwaysTerminating, $exitPoints, $throwPoints);
            ...
            $nodeCallback(new FunctionReturnStatementsNode($stmt, $gatheredReturnStatements, $gatheredYieldStatements, $statementResult, $executionEnds, $functionReflection), $functionScope);
            ...
            if ($stmt->name->toLowerString() === '__construct' || $isFromTrait) {
                foreach ($stmt->params as $param) {
                    ...
                    $nodeCallback(new ClassPropertyNode($param->var->name, $param->flags, $param->type, null, $phpDoc, $phpDocParameterTypes[$param->var->name] ?? null, \true, $isFromTrait, $param, \false, $scope->isInTrait(), $scope->getClassReflection()->isReadOnly(), \false, $scope->getClassReflection()), $methodScope);
            ...
            if ($stmt->stmts !== null) {
                ...
                $nodeCallback(new MethodReturnStatementsNode($stmt, $gatheredReturnStatements, $gatheredYieldStatements, $statementResult, $executionEnds, $classReflection, $methodReflection), $methodScope);
            }
            ...
            foreach ($stmt->props as $prop) {
                ...
                $nodeCallback(new ClassPropertyNode($propertyName, $stmt->flags, $stmt->type, $prop->default, $docComment, $phpDocType, \false, \false, $prop, $isReadOnly, $scope->isInTrait(), $scope->getClassReflection()->isReadOnly(), $isAllowedPrivateMutation, $scope->getClassReflection()), $scope);
            }
            if ($stmt->type !== null) {
     ...
     * @param callable(Node $node, Scope $scope): void $nodeCallback
     */
    public function processExprNode(Expr $expr, \PHPStan\Analyser\MutatingScope $scope, callable $nodeCallback, \PHPStan\Analyser\ExpressionContext $context) : \PHPStan\Analyser\ExpressionResult
    {
        ...
        if ($expr instanceof Variable) {
            ...
            $result = $this->processAssignVar($scope, $expr->var, $expr, $nodeCallback, $context, function (\PHPStan\Analyser\MutatingScope $scope) use($expr, $nodeCallback, $context) : \PHPStan\Analyser\ExpressionResult {
                ...
                $result = $this->processExprNode($expr->expr, $scope, $nodeCallback, $context->enterDeep());
                if ($expr instanceof Expr\AssignOp\Coalesce) {
                    return new \PHPStan\Analyser\ExpressionResult($result->getScope()->mergeWith($this->processExprNode($expr->expr, $originalScope, static function () : void {
                    }, $context->enterDeep())->getScope()), $result->hasYield(), $result->getThrowPoints());
                }
                return $result;
            ...
            if ($expr->name instanceof Expr) {
                ...
                if ($nameType->isObject()->yes() && $nameType->isCallable()->yes() && (new ObjectType(Closure::class))->isSuperTypeOf($nameType)->no()) {
                    $invokeResult = $this->processExprNode(new MethodCall($expr->name, '__invoke', $expr->getArgs(), $expr->getAttributes()), $scope, static function () : void {
                    }, $context->enterDeep());
            ...
            if ($methodReflection !== null) {
                ...
                if ($parametersAcceptor !== null) {
                    ...
                    if ($selfOutType !== null) {
                        $scope = $scope->assignExpression($expr->var, TemplateTypeHelper::resolveTemplateTypes($selfOutType, $parametersAcceptor->getResolvedTemplateTypeMap(), $parametersAcceptor instanceof ParametersAcceptorWithPhpDocs ? $parametersAcceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty(), TemplateTypeVariance::createCovariant()), $scope->getNativeType($expr->var));
                    }
                }
            ...
            $exprResult = $this->processExprNode(new MethodCall($expr->var, $expr->name, $expr->args, array_merge($expr->getAttributes(), ['virtualNullsafeMethodCall' => \true])), $nonNullabilityResult->getScope(), $nodeCallback, $context);
            ...
            $nonNullabilityResult = $this->ensureShallowNonNullability($scope, $scope, $expr->var);
            $exprResult = $this->processExprNode(new PropertyFetch($expr->var, $expr->name, array_merge($expr->getAttributes(), ['virtualNullsafePropertyFetch' => \true])), $nonNullabilityResult->getScope(), $nodeCallback, $context);
            $scope = $this->revertNonNullability($exprResult->getScope(), $nonNullabilityResult->getSpecifiedExpressions());
            return new \PHPStan\Analyser\ExpressionResult($scope, $exprResult->hasYield(), $exprResult->getThrowPoints(), static function () use($scope, $expr) : \PHPStan\Analyser\MutatingScope {
                return $scope->filterByTruthyValue($expr);
            }, static function () use($scope, $expr) : \PHPStan\Analyser\MutatingScope {
    ...
    private function processNodesForCalledMethod($node, string $fileName, MethodReflection $methodReflection, callable $nodeCallback) : void
    {
        if ($node instanceof Node) {
            $declaringClass = $methodReflection->getDeclaringClass();
            if ($node instanceof Node\Stmt\Class_ && $node->namespacedName !== null && $declaringClass->getName() === (string) $node->namespacedName && $declaringClass->getNativeReflection()->getStartLine() === $node->getStartLine()) {
                $stmts = $node->stmts;

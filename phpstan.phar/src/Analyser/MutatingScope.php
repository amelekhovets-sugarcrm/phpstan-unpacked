...
class MutatingScope implements \PHPStan\Analyser\Scope
{
    ...
    private function resolveType(string $exprString, Expr $node) : Type
    {
        ...
        if ($node instanceof Node\Expr\BinaryOp\BooleanAnd || $node instanceof Node\Expr\BinaryOp\LogicalAnd) {
            ...
            if ($this->getBooleanExpressionDepth($node->left) <= self::BOOLEAN_EXPRESSION_MAX_PROCESS_DEPTH) {
                ...
                $leftResult = $this->nodeScopeResolver->processExprNode($node->left, $this, $noopCallback, \PHPStan\Analyser\ExpressionContext::createDeep());
        ...
        if ($node instanceof Expr\Ternary) {
            ...
            $condResult = $this->nodeScopeResolver->processExprNode($node->cond, $this, $noopCallback, \PHPStan\Analyser\ExpressionContext::createDeep());
            if ($node->if === null) {
    ...
    private function createFirstClassCallable(array $variants) : Type
    {
        ...
        foreach ($variants as $variant) {
            ...
            $closureTypes[] = new ClosureType($parameters, $returnType, $variant->isVariadic(), $variant->getTemplateTypeMap(), $variant->getResolvedTemplateTypeMap(), $variant instanceof ParametersAcceptorWithPhpDocs ? $variant->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty());
        }
    ...
    public function enterForeach(self $originalScope, Expr $iteratee, string $valueName, ?string $keyName) : self
    {
        ...
        $scope = $this->assignVariable($valueName, $originalScope->getIterableValueType($iterateeType), $originalScope->getIterableValueType($nativeIterateeType));
        if ($keyName !== null) {
    ...
    public function enterForeachKey(self $originalScope, Expr $iteratee, string $keyName) : self
    {
        $iterateeType = $originalScope->getType($iteratee);
        $nativeIterateeType = $originalScope->getNativeType($iteratee);
        $scope = $this->assignVariable($keyName, $originalScope->getIterableKeyType($iterateeType), $originalScope->getIterableKeyType($nativeIterateeType));

...
class TypeSpecifier
{
    ...
    public function specifyTypesInCondition(\PHPStan\Analyser\Scope $scope, Expr $expr, \PHPStan\Analyser\TypeSpecifierContext $context, ?Expr $rootExpr = null) : \PHPStan\Analyser\SpecifiedTypes
    {
        ...
        if ($expr instanceof Instanceof_) {
            ...
            if ($expressions !== null) {
                ...
                if ($exprNode instanceof FuncCall && $exprNode->name instanceof Name && strtolower($exprNode->name->toString()) === 'gettype' && isset($exprNode->getArgs()[0]) && $constantType->isString()->yes()) {
                    return $this->specifyTypesInCondition($scope, new Expr\BinaryOp\Identical($expr->left, $expr->right), $context, $rootExpr);
                }
                if ($exprNode instanceof FuncCall && $exprNode->name instanceof Name && strtolower($exprNode->name->toString()) === 'get_class' && isset($exprNode->getArgs()[0]) && $constantType->isString()->yes()) {
                    return $this->specifyTypesInCondition($scope, new Expr\BinaryOp\Identical($expr->left, $expr->right), $context, $rootExpr);
                }
            }
            ...
            if ($leftType->isString()->yes() && $rightType->isString()->yes() || $integerType->isSuperTypeOf($leftType)->yes() && $integerType->isSuperTypeOf($rightType)->yes() || $floatType->isSuperTypeOf($leftType)->yes() && $floatType->isSuperTypeOf($rightType)->yes() || $leftType->isEnum()->yes() && $rightType->isEnum()->yes()) {
                return $this->specifyTypesInCondition($scope, new Expr\BinaryOp\Identical($expr->left, $expr->right), $context, $rootExpr);
            }
            ...
            if ($this->reflectionProvider->hasFunction($expr->name, $scope)) {
                ...
                $asserts = $functionReflection->getAsserts()->mapTypes(static function (Type $type) use($parametersAcceptor) {
                    return TemplateTypeHelper::resolveTemplateTypes($type, $parametersAcceptor->getResolvedTemplateTypeMap(), $parametersAcceptor instanceof ParametersAcceptorWithPhpDocs ? $parametersAcceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty(), TemplateTypeVariance::createInvariant());
                });
    ...
    private function specifyTypesForConstantStringBinaryExpression(Expr $exprNode, ConstantStringType $constantType, \PHPStan\Analyser\TypeSpecifierContext $context, \PHPStan\Analyser\Scope $scope, ?Expr $rootExpr) : ?\PHPStan\Analyser\SpecifiedTypes
    {
        ...
        if ($exprNode instanceof FuncCall && $exprNode->name instanceof Name && strtolower($exprNode->name->toString()) === 'gettype' && isset($exprNode->getArgs()[0])) {
            ...
            if (in_array($constantType->getValue(), ['resource', 'resource (closed)'], \true)) {
    ...
    public function resolveIdentical(Expr\BinaryOp\Identical $expr, \PHPStan\Analyser\Scope $scope, \PHPStan\Analyser\TypeSpecifierContext $context, Expr $rootExpr) : \PHPStan\Analyser\SpecifiedTypes
    {
        $leftExpr = $expr->left;
        ...
        if ($context->true() && $unwrappedLeftExpr instanceof FuncCall && $unwrappedLeftExpr->name instanceof Name && strtolower($unwrappedLeftExpr->name->toString()) === 'get_class' && isset($unwrappedLeftExpr->getArgs()[0])) {
            if ($rightType->getClassStringObjectType()->isObject()->yes()) {
                return $this->create($unwrappedLeftExpr->getArgs()[0]->value, $rightType->getClassStringObjectType(), $context, \false, $scope, $rootExpr)->unionWith($this->create($leftExpr, $rightType, $context, \false, $scope, $rootExpr));
            }
        }
        ...
        if ($expressions !== null) {
            ...
            if ($specifiedType !== null) {
                if ($exprNode instanceof AlwaysRememberedExpr) {
                    $specifiedType->unionWith($this->create($exprNode->getExpr(), $constantType, $context, \false, $scope, $rootExpr));
                }
                return $specifiedType;
            }
        }
        if ($context->true() && $unwrappedLeftExpr instanceof ClassConstFetch && $unwrappedLeftExpr->class instanceof Expr && $unwrappedLeftExpr->name instanceof Node\Identifier && $unwrappedRightExpr instanceof ClassConstFetch && $rightType instanceof ConstantStringType && strtolower($unwrappedLeftExpr->name->toString()) === 'class') {
            return $this->specifyTypesInCondition($scope, new Instanceof_($unwrappedLeftExpr->class, new Name($rightType->getValue())), $context, $rootExpr)->unionWith($this->create($leftExpr, $rightType, $context, \false, $scope, $rootExpr));
        }
        $leftType = $scope->getType($leftExpr);
        if ($context->true() && $unwrappedRightExpr instanceof ClassConstFetch && $unwrappedRightExpr->class instanceof Expr && $unwrappedRightExpr->name instanceof Node\Identifier && $unwrappedLeftExpr instanceof ClassConstFetch && $leftType instanceof ConstantStringType && strtolower($unwrappedRightExpr->name->toString()) === 'class') {
            return $this->specifyTypesInCondition($scope, new Instanceof_($unwrappedRightExpr->class, new Name($leftType->getValue())), $context, $rootExpr)->unionWith($this->create($rightExpr, $leftType, $context, \false, $scope, $rootExpr));
        }
        if ($context->false()) {
            ...
            if ($identicalType instanceof ConstantBooleanType) {
                ...
                if ($leftExpr instanceof AlwaysRememberedExpr) {
                    $leftTypes = $leftTypes->unionWith($this->create($unwrappedLeftExpr, $never, $contextForTypes, \false, $scope, $rootExpr));
                }
                if ($rightExpr instanceof AlwaysRememberedExpr) {
                    $rightTypes = $rightTypes->unionWith($this->create($unwrappedRightExpr, $never, $contextForTypes, \false, $scope, $rootExpr));
                }
        ...
        if (count($leftType->getFiniteTypes()) === 1 || $context->true() && $leftType->isConstantValue()->yes() && !$rightType->equals($leftType) && $rightType->isSuperTypeOf($leftType)->yes()) {
            $types = $this->create($rightExpr, $leftType, $context, \false, $scope, $rootExpr);
            if ($rightExpr instanceof AlwaysRememberedExpr) {
                $types = $types->unionWith($this->create($unwrappedRightExpr, $leftType, $context, \false, $scope, $rootExpr));
            }
        }
        if (count($rightType->getFiniteTypes()) === 1 || $context->true() && $rightType->isConstantValue()->yes() && !$leftType->equals($rightType) && $leftType->isSuperTypeOf($rightType)->yes()) {
            ...
            if ($leftExpr instanceof AlwaysRememberedExpr) {
                $leftTypes = $leftTypes->unionWith($this->create($unwrappedLeftExpr, $rightType, $context, \false, $scope, $rootExpr));
            }
            if ($types !== null) {
        ...
        if ($context->true()) {
            ...
            if ($rightExpr instanceof AlwaysRememberedExpr) {
                $rightTypes = $rightTypes->unionWith($this->create($unwrappedRightExpr, $leftType, $context, \false, $scope, $rootExpr));
            }
            return $leftTypes->unionWith($rightTypes);
        } elseif ($context->false()) {
            return $this->create($leftExpr, $leftType, $context, \false, $scope, $rootExpr)->normalize($scope)->intersectWith($this->create($rightExpr, $rightType, $context, \false, $scope, $rootExpr)->normalize($scope));
        }
        return new \PHPStan\Analyser\SpecifiedTypes([], [], \false, [], $rootExpr);
    }
}

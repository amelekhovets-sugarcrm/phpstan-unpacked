...
class TypeNodeResolver
{
    ...
    private $genericTypeResolvingStack = [];
    ...
    private function resolveGenericTypeNode(GenericTypeNode $typeNode, NameScope $nameScope) : Type
    {
        ...
        if (in_array($mainTypeName, ['array', 'non-empty-array'], \true)) {
            ...
            return $arrayType;
        } elseif (in_array($mainTypeName, ['list', 'non-empty-list'], \true)) {
            if (count($genericTypes) === 1) {
                // list<ValueType>
        ...
        if ($mainTypeClassName !== null) {
            ...
            if ($classReflection->isGeneric()) {
                if (in_array($mainTypeClassName, [Traversable::class, IteratorAggregate::class, Iterator::class], \true)) {
                    if (count($genericTypes) === 1) {
                        return new GenericObjectType($mainTypeClassName, [new MixedType(\true), $genericTypes[0]], null, null, [TemplateTypeVariance::createInvariant(), $variances[0]]);
                    }
                    if (count($genericTypes) === 2) {
                        return new GenericObjectType($mainTypeClassName, [$genericTypes[0], $genericTypes[1]], null, null, [$variances[0], $variances[1]]);
                    }
                }
                if ($mainTypeClassName === Generator::class) {
                    if (count($genericTypes) === 1) {
                        $mixed = new MixedType(\true);
                        return new GenericObjectType($mainTypeClassName, [$mixed, $genericTypes[0], $mixed, $mixed], null, null, [TemplateTypeVariance::createInvariant(), $variances[0], TemplateTypeVariance::createInvariant(), TemplateTypeVariance::createInvariant()]);
                    }
                    if (count($genericTypes) === 2) {
                        $mixed = new MixedType(\true);
                        return new GenericObjectType($mainTypeClassName, [$genericTypes[0], $genericTypes[1], $mixed, $mixed], null, null, [$variances[0], $variances[1], TemplateTypeVariance::createInvariant(), TemplateTypeVariance::createInvariant()]);
                    }
                }
        ...
        if ($mainType->isIterable()->yes()) {
            if ($mainTypeClassName !== null) {
                ...
                $this->genericTypeResolvingStack[$mainTypeClassName] = \true;
            }
            try {
                if (count($genericTypes) === 1) {
                    // Foo<ValueType>

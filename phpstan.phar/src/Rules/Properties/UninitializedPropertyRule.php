...
class UninitializedPropertyRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        foreach ($prematureAccess as [$propertyName, $line, $propertyNode, $file, $fileDescription]) {
            ...
            $errors[] = RuleErrorBuilder::message(sprintf('Access to an uninitialized property %s::$%s.', $classReflection->getDisplayName(), $propertyName))->line($line)->file($file, $fileDescription)->build();
        }
        return $errors;
    }
}

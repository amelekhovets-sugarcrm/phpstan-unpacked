...
use _PHPStan_c6b09fbdf\Nette\Bootstrap\Extensions\PhpExtension;
use _PHPStan_c6b09fbdf\Nette\DI\Config\Adapters\PhpAdapter;
use _PHPStan_c6b09fbdf\Nette\DI\Definitions\Statement;
use _PHPStan_c6b09fbdf\Nette\DI\Extensions\ExtensionsExtension;
use _PHPStan_c6b09fbdf\Nette\DI\Helpers;
use _PHPStan_c6b09fbdf\Nette\Schema\Context as SchemaContext;
use _PHPStan_c6b09fbdf\Nette\Schema\Elements\AnyOf;
use _PHPStan_c6b09fbdf\Nette\Schema\Elements\Structure;
use _PHPStan_c6b09fbdf\Nette\Schema\Elements\Type;
use _PHPStan_c6b09fbdf\Nette\Schema\Expect;
use _PHPStan_c6b09fbdf\Nette\Schema\Processor;
...
use _PHPStan_c6b09fbdf\Symfony\Component\Finder\Finder;
...
class ContainerFactory
{
    ...
    public function create(string $tempDirectory, array $additionalConfigFiles, array $analysedPaths, array $composerAutoloaderProjectPaths = [], array $analysedPathsFromConfig = [], string $usedLevel = CommandHelper::DEFAULT_LEVEL, ?string $generateBaselineFile = null, ?string $cliAutoloadFile = null) : \PHPStan\DependencyInjection\Container
    {
        ...
        $configurator->addDynamicParameters(['analysedPaths' => $analysedPaths, 'analysedPathsFromConfig' => $analysedPathsFromConfig, 'env' => getenv()]);
    ...
    private function detectDuplicateIncludedFiles(array $configFiles, array $loaderParameters) : array
    {
        ...
        foreach ($configFiles as $configFile) {
            ...
            $configArray = \_PHPStan_c6b09fbdf\Nette\Schema\Helpers::merge($tmpConfigArray, $configArray);
        }
    ...
    private static function getConfigFiles(FileHelper $fileHelper, \PHPStan\DependencyInjection\NeonAdapter $neonAdapter, PhpAdapter $phpAdapter, string $configFile, array $loaderParameters, ?string $generateBaselineFile) : array
    {
        ...
        if (isset($data['includes'])) {
            ...
            foreach ($includes as $include) {
                ...
                $data = \_PHPStan_c6b09fbdf\Nette\Schema\Helpers::merge($tmpConfigArray, $data);
            }
        }

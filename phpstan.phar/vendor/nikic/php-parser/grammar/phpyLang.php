...
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \_PHPStan_c6b09fbdf\PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = resolveNodes($matches['params']);
        $params = magicSplit('(?:' . \_PHPStan_c6b09fbdf\PARAMS . '|' . \_PHPStan_c6b09fbdf\ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \_PHPStan_c6b09fbdf\ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = magicSplit('(?:' . \_PHPStan_c6b09fbdf\PARAMS . '|' . \_PHPStan_c6b09fbdf\ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
...
{
    return '~' . \_PHPStan_c6b09fbdf\LIB . '(?:' . \str_replace('~', '\\~', $regex) . ')~';
}

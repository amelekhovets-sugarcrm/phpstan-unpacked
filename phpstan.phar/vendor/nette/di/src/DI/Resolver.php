...
namespace _PHPStan_c6b09fbdf\Nette\DI;

use _PHPStan_c6b09fbdf\Nette;
use _PHPStan_c6b09fbdf\Nette\DI\Definitions\Definition;
use _PHPStan_c6b09fbdf\Nette\DI\Definitions\Reference;
use _PHPStan_c6b09fbdf\Nette\DI\Definitions\Statement;
use _PHPStan_c6b09fbdf\Nette\PhpGenerator\Helpers as PhpHelpers;
use _PHPStan_c6b09fbdf\Nette\Utils\Arrays;
use _PHPStan_c6b09fbdf\Nette\Utils\Callback;
use _PHPStan_c6b09fbdf\Nette\Utils\Reflection;
use _PHPStan_c6b09fbdf\Nette\Utils\Strings;
use _PHPStan_c6b09fbdf\Nette\Utils\Validators;
use ReflectionClass;
/**
 * Services resolver
 * @internal
...
class Resolver
{
    ...
    public function resolveEntityType(Statement $statement) : ?string
    {
        ...
        if (\is_array($entity)) {
            ...
            if ($type && !\in_array($type->getSingleName(), ['object', 'mixed'], \true)) {
                ...
                return Helpers::ensureClassType($type, \sprintf('return type of %s()', Callback::toString($entity)), \true);
            }
            return null;
            ...
            if (!\class_exists($entity)) {
                throw new ServiceCreationException(\sprintf(\interface_exists($entity) ? "Interface %s can not be used as 'create' or 'factory', did you mean 'implement'?" : "Class '%s' not found.", $entity));
            }
            return $entity;
        }
        return null;
    ...
    private function convertReferences(array $arguments) : array
    {
        \array_walk_recursive($arguments, function (&$val) : void {
            if (\is_string($val) && \strlen($val) > 1 && $val[0] === '@' && $val[1] !== '@') {
                ...
                } elseif (\preg_match('#^[A-Z][a-zA-Z0-9_]*$#D', $pair[1], $m)) {
                    // @service::CONSTANT
    ...
    public static function autowireArguments(\ReflectionFunctionAbstract $method, array $arguments, callable $getter) : array
    {
        ...
        foreach ($method->getParameters() as $num => $param) {
            ...
            if ($param->isVariadic()) {
                if ($useName && Arrays::some($arguments, function ($val, $key) {
                    return \is_int($key);
                })) {
                    throw new ServiceCreationException(\sprintf('Cannot use positional argument after named or omitted argument in %s.', Reflection::toString($param)));
                } elseif (\array_key_exists($paramName, $arguments)) {
                    if (!\is_array($arguments[$paramName])) {
                        throw new ServiceCreationException(\sprintf('Parameter %s must be array, %s given.', Reflection::toString($param), \gettype($arguments[$paramName])));
                    }
                ...
                unset($arguments[$key], $arguments[$num]);
                // unset $num to enable overwriting in configuration
                ...
                if ($param->isOptional()) {
                    ...
                    $res[$num] = null;
                    \trigger_error(\sprintf('The parameter %s should have a declared value in the configuration.', Reflection::toString($param)), \E_USER_DEPRECATED);
                }
            } else {
                $res[$num] = $param->isDefaultValueAvailable() ? Reflection::getParameterDefaultValue($param) : null;
                if (!$param->isOptional()) {
                    \trigger_error(\sprintf('The parameter %s should have a declared value in the configuration.', Reflection::toString($param)), \E_USER_DEPRECATED);
                }
            }

...
use _PHPStan_c6b09fbdf\React\Http\Message\Response;
use _PHPStan_c6b09fbdf\React\Http\Message\ResponseException;
use _PHPStan_c6b09fbdf\React\Promise\Deferred;
use _PHPStan_c6b09fbdf\React\Promise\Promise;
use _PHPStan_c6b09fbdf\React\Promise\PromiseInterface;
use _PHPStan_c6b09fbdf\React\Stream\ReadableStreamInterface;
use _PHPStan_c6b09fbdf\RingCentral\Psr7\Uri;
/**
 * @internal
 */
class Transaction
{
    ...
    public function bufferResponse(ResponseInterface $response, Deferred $deferred, ClientRequestState $state)
    {
        ...
        if ($size !== null && $size > $this->maximumSize) {
            $body->close();
            return \_PHPStan_c6b09fbdf\React\Promise\reject(new \OverflowException('Response body size of ' . $size . ' bytes exceeds maximum of ' . $this->maximumSize . ' bytes', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        // body is not streaming => already buffered
        if (!$body instanceof ReadableStreamInterface) {
            return \_PHPStan_c6b09fbdf\React\Promise\resolve($response);
        }
        /** @var ?\Closure $closer */
        $closer = null;

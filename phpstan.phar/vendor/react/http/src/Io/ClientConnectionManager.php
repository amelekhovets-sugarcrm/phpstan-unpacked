...
class ClientConnectionManager
{
    ...
    public function connect(UriInterface $uri)
    {
        ...
        if ($scheme !== 'https' && $scheme !== 'http') {
            return \_PHPStan_c6b09fbdf\React\Promise\reject(new \InvalidArgumentException('Invalid request URL given'));
        }
        $port = $uri->getPort();
        if ($port === null) {
        ...
        foreach ($this->idleConnections as $id => $connection) {
            if ($this->idleUris[$id] === $uri) {
                ...
                return \_PHPStan_c6b09fbdf\React\Promise\resolve($connection);
            }
        }

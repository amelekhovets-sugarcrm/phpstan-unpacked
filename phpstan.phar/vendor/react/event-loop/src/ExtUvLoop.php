...
namespace _PHPStan_c6b09fbdf\React\EventLoop;

use _PHPStan_c6b09fbdf\React\EventLoop\Tick\FutureTickQueue;
use _PHPStan_c6b09fbdf\React\EventLoop\Timer\Timer;
...
final class ExtUvLoop implements LoopInterface
{
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = \_PHPStan_c6b09fbdf\uv_signal_init($this->uv);
            \_PHPStan_c6b09fbdf\uv_signal_start($this->signalEvents[$signal], function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
        }
    }
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \_PHPStan_c6b09fbdf\uv_poll_init_socket($this->uv, $stream);
        }
